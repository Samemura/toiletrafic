...
class BoothUsage < ApplicationRecord
  EXIPRE_PERIOD = 8.days.ago.at_end_of_day
  ...
  class << self
    ...
    def average_by_hour(period=EXIPRE_PERIOD..1.day.ago.at_end_of_day, hour_range: 8..19)
      days = ((period.last-period.first).to_f/60/60/24) - self.weekdays(period)
    ...
    def weekdays(period)
      period.first.to_datetime.step(period.last.to_datetime, 1).inject(0){|sum, p| sum + ([0,6].exclude?(p.wday) ? 1 : 0)}
    end
  end
end
